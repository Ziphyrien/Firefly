...
import type { SettingsSection } from "@/ui/lib/search-state";
import { BadgeCheck, Sparkles, Palette, Receipt, Globe, Database, HelpCircle } from "lucide-react";
import { CostsPanel } from "@/ui/components/costs-panel";
import { DataSettings } from "@/ui/components/data-settings";
import {
  ExtensionsSettings,
  type ExtensionSettingsEntry,
} from "@/ui/components/extensions-settings";
import { ProviderSettings } from "@/ui/components/provider-settings";
import { ProxySettings } from "@/ui/components/proxy-settings";
import { AppearanceSettings } from "@/ui/components/appearance-settings";
...
import { useSelectedSessionSummary } from "@/pi/hooks/use-selected-session-summary";
...
export function AppSettingsDialog(props: {
  ...
  return (
    ...
    >
      <DialogContent
        className="flex max-h-[90dvh] min-h-0 w-full max-w-[calc(100%-2rem)] flex-col gap-0 overflow-hidden p-0 sm:max-w-[min(100%-2rem,36rem)] md:h-[620px] md:max-h-[620px] md:min-h-[620px] md:max-w-5xl"
